...
contract DssProxyActions is Common {
    ...
    function _getWipeDart(address vat, uint256 dai, address urn, bytes32 ilk) internal view returns (int256 dart) {
    ...
    function wipe(address manager, address daiJoin, uint256 cdp, uint256 wad) public {
        address vat = ManagerLike(manager).vat();
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
    ...
    function safeWipe(address manager, address daiJoin, uint256 cdp, uint256 wad, address owner) public {
        require(ManagerLike(manager).owns(cdp) == owner, "owner-missmatch");
        wipe(manager, daiJoin, cdp, wad);
    }

    function wipeAll(address manager, address daiJoin, uint256 cdp) public {
        address vat = ManagerLike(manager).vat();
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint256 art) = VatLike(vat).urns(ilk, urn);
    ...
    function safeWipeAll(address manager, address daiJoin, uint256 cdp, address owner) public {
        require(ManagerLike(manager).owns(cdp) == owner, "owner-missmatch");
    ...
    function wipeAllAndFreeETH(address manager, address ethJoin, address daiJoin, uint256 cdp, uint256 wadC) public {
        address vat = ManagerLike(manager).vat();
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint256 art) = VatLike(vat).urns(ilk, urn);
    ...
    function wipeAllAndFreeGem(address manager, address gemJoin, address daiJoin, uint256 cdp, uint256 amtC) public {
        address vat = ManagerLike(manager).vat();
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint256 art) = VatLike(vat).urns(ilk, urn);
...
contract DssProxyActionsEnd is Common {
    ...
    function pack(address daiJoin, address end, uint256 wad) public {
        daiJoin_join(daiJoin, address(this), wad);
...
contract DssProxyActionsDsr is Common {
    function join(address daiJoin, address pot, uint256 wad) public {
        VatLike vat = DaiJoinLike(daiJoin).vat();
    ...
    function exit(address daiJoin, address pot, uint256 wad) public {
        VatLike vat = DaiJoinLike(daiJoin).vat();
