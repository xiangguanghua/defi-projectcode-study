...
contract ProxyCalls {
    ...
    function lockGem(address, address, uint256, uint256, bool) public {
        proxy.execute(dssProxyActions, msg.data);
    }

    function safeLockGem(address, address, uint256, uint256, bool, address) public {
        proxy.execute(dssProxyActions, msg.data);
    }
    ...
    function openLockETHAndDraw(address, address, address, address, bytes32, uint256)
        public
        payable
        returns (uint256 cdp)
    {
    ...
    function lockGemAndDraw(address, address, address, address, uint256, uint256, uint256, bool) public {
        proxy.execute(dssProxyActions, msg.data);
    }

    function openLockGemAndDraw(address, address, address, address, bytes32, uint256, uint256, bool)
        public
        returns (uint256 cdp)
    {
        bytes memory response = proxy.execute(dssProxyActions, msg.data);
    ...
    function openLockGNTAndDraw(address, address, address, address, bytes32, uint256, uint256)
        public
        returns (address bag, uint256 cdp)
    {
        bytes memory response = proxy.execute(dssProxyActions, msg.data);
        assembly {
    ...
    function end_freeETH(address a, address b, address c, uint256 d) public {
        proxy.execute(
            dssProxyActionsEnd, abi.encodeWithSignature("freeETH(address,address,address,uint256)", a, b, c, d)
        );
    }

    function end_freeGem(address a, address b, address c, uint256 d) public {
        proxy.execute(
            dssProxyActionsEnd, abi.encodeWithSignature("freeGem(address,address,address,uint256)", a, b, c, d)
        );
    }
    ...
    function end_cashETH(address a, address b, bytes32 c, uint256 d) public {
        proxy.execute(
            dssProxyActionsEnd, abi.encodeWithSignature("cashETH(address,address,bytes32,uint256)", a, b, c, d)
        );
    }

    function end_cashGem(address a, address b, bytes32 c, uint256 d) public {
        proxy.execute(
            dssProxyActionsEnd, abi.encodeWithSignature("cashGem(address,address,bytes32,uint256)", a, b, c, d)
        );
    }
...
contract FakeUser {
    function doGive(DssCdpManager manager, uint256 cdp, address dst) public {
...
contract DssProxyActionsTest is DssDeployTestBase, ProxyCalls {
    ...
    function setUp() public {
        ...
        pipGNT.poke(bytes32(uint256(100 ether))); // Price 100 DAI = 1 GNT
        this.file(address(spotter), "GNT", "mat", uint256(1500000000 ether)); // Liquidation ratio 150%
        this.file(address(vat), bytes32("GNT"), bytes32("line"), uint256(10000 * 10 ** 45));
        spotter.poke("GNT");
        (,, spot,,) = vat.ilks("GNT");
    ...
    function testLockGem() public {
        col.mint(5 ether);
        uint256 cdp = this.open(address(manager), "COL", address(proxy));
    ...
    function testLockGemGNT() public {
        uint256 cdp = this.open(address(manager), "GNT", address(proxy));
        assertEq(ink("GNT", manager.urns(cdp)), 0);
        uint256 prevBalance = gnt.balanceOf(address(this));
    ...
    function testOpenLockETHAndDraw() public {
        uint256 initialBalance = address(this).balance;
        assertEq(dai.balanceOf(address(this)), 0);
        uint256 cdp = this.openLockETHAndDraw.value(2 ether)(
    ...
    function testLockGemAndDraw() public {
        ...
        assertEq(dai.balanceOf(address(this)), 0);
        this.lockGemAndDraw(
            address(manager), address(jug), address(colJoin), address(daiJoin), cdp, 2 ether, 10 ether, true
        );
        assertEq(ink("COL", manager.urns(cdp)), 2 ether);
    ...
    function testLockGemDGDAndDraw() public {
        ...
        uint256 prevBalance = dgd.balanceOf(address(this));
        this.lockGemAndDraw(
            address(manager), address(jug), address(dgdJoin), address(daiJoin), cdp, 3 * 10 ** 9, 50 ether, true
        );
    ...
    function testLockGemGNTAndDraw() public {
        ...
        this.lockGemAndDraw(
            address(manager), address(jug), address(gntJoin), address(daiJoin), cdp, 3 ether, 50 ether, false
        );
    ...
    function testOpenLockGemAndDraw() public {
        ...
        col.approve(address(proxy), 2 ether);
        assertEq(dai.balanceOf(address(this)), 0);
        uint256 cdp = this.openLockGemAndDraw(
            address(manager), address(jug), address(colJoin), address(daiJoin), "COL", 2 ether, 10 ether, true
        );
        assertEq(ink("COL", manager.urns(cdp)), 2 ether);
        assertEq(dai.balanceOf(address(this)), 10 ether);
        assertEq(col.balanceOf(address(this)), 3 ether);
    ...
    function testOpenLockGemGNTAndDraw() public {
        ...
        gnt.transfer(bag, 2 ether);
        uint256 cdp = this.openLockGemAndDraw(
            address(manager), address(jug), address(gntJoin), address(daiJoin), "GNT", 2 ether, 10 ether, false
        );
        assertEq(ink("GNT", manager.urns(cdp)), 2 ether);
        assertEq(dai.balanceOf(address(this)), 10 ether);
    }
    ...
    function testOpenLockGemGNTAndDrawSafe() public {
        ...
        (address bag, uint256 cdp) = this.openLockGNTAndDraw(
    ...
    function testOpenLockGemGNTAndDrawSafeTwice() public {
        ...
        (address bag2, uint256 cdp2) = this.openLockGNTAndDraw(
            address(manager), address(jug), address(gntJoin), address(daiJoin), "GNT", 2 ether, 10 ether
        );
        assertEq(address(bag), gntJoin.bags(address(proxy)));
    ...
    function testShift() public {
        uint256 cdpSrc = this.open(address(manager), "ETH", address(proxy));
        this.lockETHAndDraw.value(1 ether)(
            address(manager), address(jug), address(ethJoin), address(daiJoin), cdpSrc, 50 ether
        );

        uint256 cdpDst = this.open(address(manager), "ETH", address(proxy));
    ...
    function testExitGemAfterFlip() public {
        ...
        this.lockGemAndDraw(
            address(manager), address(jug), address(colJoin), address(daiJoin), cdp, 1 ether, 40 ether, true
        );
    ...
    function testExitDGDAfterFlip() public {
        ...
        this.lockGemAndDraw(
            address(manager), address(jug), address(dgdJoin), address(daiJoin), cdp, 1 * 10 ** 9, 30 ether, true
        );
    ...
    function testEnd() public {
        ...
        col.mint(1 ether);
        col.approve(address(proxy), 1 ether);
        uint256 cdp2 = this.openLockGemAndDraw(
            address(manager), address(jug), address(colJoin), address(daiJoin), "COL", 1 ether, 5 ether, true
        );
        dgd.approve(address(proxy), 1 * 10 ** 9);
        uint256 cdp3 = this.openLockGemAndDraw(
            address(manager), address(jug), address(dgdJoin), address(daiJoin), "DGD", 1 * 10 ** 9, 5 ether, true
        );
