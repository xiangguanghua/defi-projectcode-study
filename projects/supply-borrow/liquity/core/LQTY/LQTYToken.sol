...
contract LQTYToken is CheckContract, ILQTYToken {
    ...
    constructor(
        ...
        uint256 multisigEntitlement =
            _1_MILLION.mul(100).sub(bountyEntitlement).sub(depositorsAndFrontEndsEntitlement).sub(_lpRewardsEntitlement);
    ...
    function _buildDomainSeparator(bytes32 typeHash, bytes32 name_, bytes32 version_) private view returns (bytes32) {
        return keccak256(abi.encode(typeHash, name_, version_, _chainID(), address(this)));
    }
