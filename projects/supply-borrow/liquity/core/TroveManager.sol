...
contract TroveManager is LiquityBase, Ownable, CheckContract, ITroveManager {
    ...
    //标识合约名称（"TroveManager"），用于调试或前端显示
    string public constant NAME = "TroveManager";

    // ​依赖的外部合约
    // --- Connected contract declarations ---
    address public borrowerOperationsAddress; // 用户操作 Trove 的入口合约（如开仓、调整抵押率）
    IStabilityPool public override stabilityPool; //稳定池，用于吸收清算的债务并支付奖励。
    address gasPoolAddress;
    ICollSurplusPool collSurplusPool;
    ILUSDToken public override lusdToken; // 协议稳定币（LUSD）
    ILQTYToken public override lqtyToken; // 治理代币（LQTY）
    ILQTYStaking public override lqtyStaking;

    // A doubly linked list of Troves, sorted by their sorted by their collateral ratios
    ISortedTroves public sortedTroves; // 按抵押率（ICR）排序的 Trove 双向链表，优化清算和赎回的效率。
     ...
     */
    // ​12 小时半衰期的衰减因子，用于动态调整赎回费率（随时间递减）
    ...
    */
    ...
    enum Status {
        nonExistent,
        active, // 正常
        closedByOwner, // 用户主动关闭
        closedByLiquidation, //被清算
        closedByRedemption // 被赎回

    }

    // Store the necessary data for a trove
    struct Trove {
        uint256 debt; // 债务金额（LUSD）
        uint256 coll; // 抵押品金额（如 ETH）
        uint256 stake; // 在系统内的权益份额
        Status status; // 状态
        uint128 arrayIndex; // 在排序链表中的位置
    }
    ...
    **/
    ...
    struct LocalVariables_LiquidationSequence {
        uint256 remainingLUSDInSPForOffsets;
        uint256 i;
        uint256 ICR;
        address user;
    ...
    function _liquidateNormalMode(
        ...
        ) = _getOffsetAndRedistributionVals(singleLiquidation.entireTroveDebt, collToLiquidate, _LUSDInSPForOffsets);
    ...
    function _liquidateRecoveryMode(
        ...
        if (_ICR <= _100pct) {
            ...
            (
                ...
                singleLiquidation.entireTroveDebt, vars.collToLiquidate, _LUSDInSPForOffsets
            );
        ...
        */
            ...
            // if (_ICR >= MCR && ( _ICR >= _TCR || singleLiquidation.entireTroveDebt > _LUSDInSPForOffsets))
    ...
    */
    function _getOffsetAndRedistributionVals(uint256 _debt, uint256 _coll, uint256 _LUSDInSPForOffsets)
        internal
        pure
    ...
    function liquidateTroves(uint256 _n) external override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalsFromLiquidateTrovesSequence_RecoveryMode(
                contractsCache, vars.price, vars.LUSDInSPForOffsets, _n
            );
        } else {
            // if !vars.recoveryModeAtStart
            totals = _getTotalsFromLiquidateTrovesSequence_NormalMode(
                contractsCache.activePool, contractsCache.defaultPool, vars.price, vars.LUSDInSPForOffsets, _n
            );
        }
    ...
    */
        ...
        for (vars.i = 0; vars.i < _n && vars.user != firstUser; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                singleLiquidation = _liquidateRecoveryMode(
                    ...
                    vars.user,
                    vars.ICR,
                    vars.remainingLUSDInSPForOffsets,
                    TCR,
                    _price
                );
                ...
                singleLiquidation = _liquidateNormalMode(
                    _contractsCache.activePool, _contractsCache.defaultPool, vars.user, vars.remainingLUSDInSPForOffsets
                );
    ...
    function _getTotalsFromLiquidateTrovesSequence_NormalMode(
        ...
        for (vars.i = 0; vars.i < _n; vars.i++) {
            ...
            if (vars.ICR < MCR) {
                singleLiquidation =
                    _liquidateNormalMode(_activePool, _defaultPool, vars.user, vars.remainingLUSDInSPForOffsets);
    ...
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalFromBatchLiquidate_RecoveryMode(
                activePoolCached, defaultPoolCached, vars.price, vars.LUSDInSPForOffsets, _troveArray
            );
        } else {
            //  if !vars.recoveryModeAtStart
    ...
    function _getTotalFromBatchLiquidate_RecoveryMode(
        ...
        for (vars.i = 0; vars.i < _troveArray.length; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                singleLiquidation = _liquidateRecoveryMode(
                    _activePool, _defaultPool, vars.user, vars.ICR, vars.remainingLUSDInSPForOffsets, TCR, _price
                );
