...
contract ATokenMock {
    ...
    event AssetConfigUpdated(
        address indexed asset, address indexed reward, uint256 emission, uint256 distributionEnd, uint256 assetIndex
    );

    event Accrued(
        address indexed asset, address indexed user, uint256 assetIndex, uint256 userIndex, uint256 rewardsAccrued
    );
    ...
    function handleActionOnAic(address user, uint256 totalSupply_, uint256 userBalance) external {
        _aic.handleAction(user, totalSupply_, userBalance);
    }

    function doubleHandleActionOnAic(address user, uint256 totalSupply_, uint256 userBalance) external {
        _aic.handleAction(user, totalSupply_, userBalance);
        _aic.handleAction(user, totalSupply_, userBalance);
    }
