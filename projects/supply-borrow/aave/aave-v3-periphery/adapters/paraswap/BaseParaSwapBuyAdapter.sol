...
abstract contract BaseParaSwapBuyAdapter is BaseParaSwapAdapter {
    ...
    function _buyOnParaSwap(
...

        {
            ...
            uint256 expectedMaxAmountToSwap = amountToReceive.mul(toAssetPrice.mul(10 ** fromAssetDecimals)).div(
                fromAssetPrice.mul(10 ** toAssetDecimals)
        ...
        if (toAmountOffset != 0) {
            ...
            require(
                toAmountOffset >= 4 && toAmountOffset <= buyCalldata.length.sub(32), "TO_AMOUNT_OFFSET_OUT_OF_RANGE"
            );
